...
export default Component.extend({
    ...
    toolbar: computed('images.[]', function () {
        ...
        if (!isEmpty(this.images)) {
            items.push({
                title: this.intl.t('koenig.gallery.Add images'),
                icon: 'koenig/kg-add',
    ...
    actions: {
        ...
        uploadFailed(uploadResult) {
            ...
            this.set('errorMessage', this.intl.t('koenig.gallery.{fileName} failed to upload', {fileName}));
        },
        handleErrors(errors) {
            let errorMssg = ((errors[0] && errors[0].message)) || this.intl.t('koenig.gallery.Some images failed to upload');
    ...
    _startUpload(files = []) {
        ...
        if (strippedFiles.length < files.length) {
            this.set('errorMessage', this.intl.t('koenig.gallery.Galleries are limited to 9 images'));
        }
